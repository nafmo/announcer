...
Uses
  Crc32, AnStr, Globals, Dos, MkMisc, StrUtil;
{$IFDEF OS2}
Uses BSEDos, Os2Dt;
{$ENDIF}
...
Begin
  ...
  If IoResult <> 0 then begin                   { Gick inte att ”ppna? }
    ...
    If IoResult = 0 then begin                  { Gick att skapa       }
      ...
      Writeln;
      If i <> 10 then begin
        Writeln(StrErrIds, ServerFileName);
        Log^.LogLine('!')^.LogStr(StrErrIds + ServerFileName)^.LogLn;
  ...
  If ServerDat.Signature <> ServerSignature then begin { fel signatur }
    Log^.LogLine('!')^.LogStr(StrErrId2)^.LogLn;
    Seek(ServerFile, 0);
    Truncate(ServerFile);
    CreateFile;
  end;
  If ServerDat.Crc32 <> ServerSignature then begin { Kontrollera CRC32 }
    ...
    If ServerDat.Crc32 <> crc then begin { CRC-fel }
      {$IFDEF MSDOS}
      Log^.LogLine('!')^.LogStr(StrErrId3 + ' ' + LongWord(ServerDat.Crc32) +
                                ' <> ' + LongWord(crc))^.LogLn;
      {$ELSE}
      Log^.LogLine('!')^.LogStr(StrErrId3)^.LogLn;
      {$ENDIF}
      Truncate(ServerFile);
      CreateFile;
    end;
